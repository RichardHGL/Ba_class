...
class worridor{
	public:
		...
		void get_weapon(const worridor& wp){
			if(!has_arrow()&&wp.has_arrow()){
				weapon[weapon_num].type=2;
				weapon[weapon_num].force=0;
				weapon[weapon_num].time=wp.has_arrow();
				weapon_num++;
			}
			if(!has_sword()&&wp.has_sword()){
				weapon[weapon_num].type=0;
				weapon[weapon_num].force=wp.sword(0);
				weapon[weapon_num].time=0;
				weapon_num++;
			}
			if(!has_bomb()&&wp.has_bomb()){
				weapon[weapon_num].type=1;
				weapon[weapon_num].force=0;
				weapon[weapon_num].time=0;
				weapon_num++;
			}
		...
		int battle_win(worridor wp1,worridor wp2,int adv){//规定wp1是红，wp2是蓝 
			if(wp2.strenth<=0&&wp1.strenth>0) return 1;
			if(wp1.strenth<=0&&wp2.strenth>0) return 2;
			if(adv==0){
				int attack=wp1.force+wp1.sword(0);
				if(attack>=wp2.strenth) return 1;
				if(wp2.type==2) return 0;
				int b_attack=wp2.force/2+wp2.sword(0);
				if(b_attack>=wp1.strenth) return 2; 
			} 
			else if(adv==1){
				int attack=wp2.force+wp2.sword(0);
				if(attack>=wp1.strenth) return 2;
				if(wp1.type==2) return 0;
				int b_attack=wp1.force/2+wp1.sword(0);
		...
		int battle(worridor &wp,int p,int adv,int hour,int minute){//0代表初态均死或无胜负
			...
			if(adv==0){
				...
				if(strenth<=0){
					...
					if(wp.type==4) wp.get_weapon(this);
					cout<<" red ";
					output(type);
					cout<<" "<<num;
					cout<<" was killed in city "<<p<<endl;
		...
		int usebomb(worridor &wp,int p,int adv){
			...
			if(has_bomb()&&battle_win(this,wp,adv)==2){
